...
export const Route = createFileRoute("/approvals")({
  head: () => ({
    meta: [
      { title: "Approval Engine — CROTON IT SOLUTIONS" },
      {
        name: "description",
        content:
          "Hiring Manager and HR review queues: record decisions with notes and automatically trigger the next lifecycle stage such as Hiring Manager Approved.",
      },
      { property: "og:title", content: "Approval Engine — CROTON IT SOLUTIONS" },
      {
        property: "og:description",
        content: "Two-tier approvals with decision notes and automatic lifecycle progression.",
      },
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b border-border bg-surface px-6 py-6">
        ...
        <h1 className="mt-1 text-4xl leading-none">Approvals</h1>
            ...
            <TabsList>
              {queues.map((q) => (
                <TabsTrigger key={q.key} value={q.key}>
                  {q.label} ({approvals.filter((a) => a.approverRole === q.key && a.status === "pending").length})
                </TabsTrigger>
              ))}
            </TabsList>
            {queues.map((q) => (
