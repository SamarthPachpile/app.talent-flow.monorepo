...
export const CompanyJobsList: React.FC<CompanyJobsListProps> = ({
  ...
  const [viewingCtcJob, setViewingCtcJob] = useState<JobPosting | null>(null);

  // Subscribe to real-time updates from MongoDB Atlas 'jobs' collection
  useEffect(() => {
    setIsLoading(true);
  ...
  const handleDeleteJob = async (job: JobPosting) => {
    const result = await Swal.fire({
      ...
      text: "This will permanently delete this job requisition from the database.",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      ...
      if (res.success) {
        toast.success(`Job "${job.title}" deleted from MongoDB database.`);
      } else {
    ...
    <div className="space-y-6 animate-fadeIn font-sans pb-12">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
        ...
        <div className="p-4 rounded-2xl bg-white dark:bg-slate-850 border border-slate-200/90 dark:border-slate-800 shadow-xs flex items-center gap-3">
          ...
          <div>
            <div className="text-xl font-bold text-slate-900 dark:text-slate-100">{draftCount}</div>
            <div className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">
              Draft Positions
            </div>
          </div>
      ...
      {isLoading ? (
        <div className="clip-path-card bg-white dark:bg-slate-850 border border-slate-200/90 dark:border-slate-800 p-12 text-center text-slate-400 text-xs">
          ...
          <span>Loading company job postings from MongoDB Atlas...</span>
        </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              ...
              selectedWorkplace !== "all"
                ...
                : "Create your first job posting stored in MongoDB Atlas to start receiving candidate applications."}
            </p>
          </div>
          <button
            type="button"
            ...
            >
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5" onClick={(e) => e.stopPropagation()}>
                  ...
                  {/* View CTC Structure Document Button */}
                  <button
                    type="button"
                    onClick={() => setViewingCtcJob(job)}
                    className="px-2.5 py-1 text-xs font-semibold rounded-lg bg-sky-50 dark:bg-sky-950/40 text-sky-700 dark:text-sky-300 hover:bg-sky-100 dark:hover:bg-sky-900/50 border border-sky-200/80 dark:border-sky-800/60 transition-colors flex items-center gap-1 cursor-pointer"
                    title="View internal CTC Compensation Breakdown"
                  >
      ...
      {/* CTC Breakdown View / Print Modal */}
      {viewingCtcJob && (
        <CtcBreakdownViewModal
          isOpen={Boolean(viewingCtcJob)}
          onClose={() => setViewingCtcJob(null)}
          breakdown={
            viewingCtcJob.ctcBreakdown ||
            calculateDefaultCtcBreakdown(
              viewingCtcJob.salaryMax
                ? Number(viewingCtcJob.salaryMax)
                : viewingCtcJob.salaryMin
                  ? Number(viewingCtcJob.salaryMin)
                  : 770000,
              viewingCtcJob.companyName || companyName,
              "01-Jun-2026",
              "Onroll (Code on Wages)",
            )
          }
          jobTitle={viewingCtcJob.title}
          companyName={viewingCtcJob.companyName || companyName}
        />
      )}
    </div>
  );
