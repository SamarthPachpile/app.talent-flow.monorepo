  ...
  const handleNext = () => {
    if (state.currentStep === 1) {
      if (!state.profile.name?.trim()) {
        toast.error(
          "Company Name is mandatory to initialize your workspace and generate your Candidate Portal.",
        );
        return;
      }
      if (!state.profile.subdomain?.trim() && !state.profile.domain?.trim()) {
        toast.error("Company Subdomain / Workspace URL slug is mandatory.");
        return;
      }
      if (!state.profile.industry?.trim()) {
        toast.error("Please select your Company Industry.");
        return;
      }
      if (!state.admin?.fullName?.trim()) {
        toast.error("Admin Full Name is mandatory.");
        return;
      }
    }
        ...
        <AnimatePresence mode="wait">
          ...
          {state.currentStep === 17 && (
            ...
            >
              ...
              {/* Dedicated Live Candidate Career Portal Box */}
              {(() => {
                const companySlug =
                ...
                return (
                  <div className="p-6 rounded-3xl bg-gradient-to-br from-ember/10 via-background to-amber-500/10 border-2 border-ember/30 space-y-4 shadow-md">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border/80 pb-3">
                      <div className="flex items-center gap-3">
                        <div className="p-2.5 rounded-2xl bg-ember/20 text-ember border border-ember/30">
                          <Globe className="size-6" />
                        </div>
                        <div>
                          <h3 className="font-display font-bold text-base text-foreground flex items-center gap-2">
                            <span>Live Candidate Career Portal</span>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Share this link with job applicants to apply and track their recruitment
                            pipeline.
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-3 bg-surface p-3.5 rounded-2xl border border-border">
                      <input
                        type="text"
                        readOnly
                        value={liveCandidatePortalUrl}
                      ...
                      <div className="flex items-center gap-2 w-full sm:w-auto shrink-0">
                        <button
                          ...
                          onClick={() => {
                            if (typeof navigator !== "undefined" && navigator.clipboard) {
                              navigator.clipboard.writeText(liveCandidatePortalUrl);
                              toast.success("Candidate Portal URL copied to clipboard!");
                            }
                          }}
                          className="flex-1 sm:flex-initial px-3.5 py-2 rounded-xl bg-surface border border-border hover:bg-accent text-foreground text-xs font-semibold flex items-center justify-center gap-1.5 cursor-pointer transition-colors"
                        >
                        ...
                        <a
                          href={liveCandidatePortalUrl}
                          target="_blank"
                          rel="noreferrer"
                          className="flex-1 sm:flex-initial px-4 py-2 rounded-xl bg-ember text-ember-foreground text-xs font-bold flex items-center justify-center gap-1.5 shadow-sm hover:opacity-90 transition-opacity"
                        >
              ...
              <div className="pt-4 text-center">
                <button
                  onClick={handleNext}
                  className="w-full sm:w-auto px-10 py-4 rounded-2xl bg-gradient-to-r from-ember via-amber-500 to-orange-500 text-ember-foreground font-display font-semibold text-lg shadow-xl shadow-ember/25 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 mx-auto"
                >
