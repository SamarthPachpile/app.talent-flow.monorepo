...
import { companyAuth } from "./firebase";
...
export const FirebaseAuthService = {
  ...
  async signInWithGoogle(): Promise<AuthResult> {
    try {
      ...
      const userCredential = await signInWithPopup(companyAuth, provider);
  ...
  async signUp(email: string, password: string, displayName?: string): Promise<AuthResult> {
    try {
      const userCredential = await createUserWithEmailAndPassword(companyAuth, email, password);
  ...
  async sendVerificationEmail(
    ...
    try {
      const user = companyAuth.currentUser;
      if (user) {
  ...
  async signIn(email: string, password: string): Promise<AuthResult> {
    try {
      const userCredential = await signInWithEmailAndPassword(companyAuth, email, password);
      return { user: userCredential.user };
    } catch (err) {
  ...
  async signOut(): Promise<{ success: boolean; error?: string }> {
    try {
      await firebaseSignOut(companyAuth);
      return { success: true };
    } catch (err: unknown) {
  ...
  onAuthChange(callback: (user: User | null) => void) {
    return onAuthStateChanged(companyAuth, callback);
  },
  ...
  getCurrentUser(): User | null {
    return companyAuth?.currentUser || null;
  },
};
