...
export default defineConfig(({ mode }) => {
  ...
  const portStr =
    env.VITE_PORT_CROTON_IT_SOLUTIONS || env.VITE_CROTON_PORT || env.VITE_PORT || env.PORT;

  if (!portStr) {
    throw new Error("Port for croton-it-solutions must be specified in the .env file!");
  }
  ...
  return {
    ...
    resolve: {
      alias: {
        ...
        "@croton-it-solutions": path.resolve(import.meta.dirname, "./src"),
        "@talent-flow/croton-it-solutions": path.resolve(import.meta.dirname, "./src"),
